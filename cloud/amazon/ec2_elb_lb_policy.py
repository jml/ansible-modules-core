...
description:
  - This module makes it possible to add or remove policies on EC2 Elastic
    ...
    policy configuration; the presence of the policy is confirmed by the
    checking for a policy of that name on the load balancer.  If a policy of
    the given name exists in the load balancer, it's considered present,
    whether or not the configuration matches.
version_added: "1.9"
...
class LBPolicyManager(object):
    ...
    def __init__(self, lb_name, region, boto_params={}):
        ...
        if not self.lb_name:
            raise ValueError(
                'Load balancer name is a required parameter.')
...
class LBPolicyModule(object):
    ...
    def check_policy(self):
        ...
        if not isinstance(self.policy, Mapping) or not self.policy:
            raise TypeError(
                '''
The 'policy' parameter should be specified as a dictionary in the format:
